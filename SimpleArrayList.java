...
public class SimpleArrayList<E> implements RandomAccess, Cloneable, Serializable {
    ...
    public SimpleArrayList(int theSize) {
        if (theSize < 0) {
            throw new IllegalArgumentException("默认的大小" + theSize);
        } else {
            elementData = new Object[theSize];
        }
        System.out.println("使用简单的arrayList");
    }
    ...
    public boolean change(Object o, E e) {
        if (contains(o)) {
            change(indexOf(o), e);
            return true;
        } else {
